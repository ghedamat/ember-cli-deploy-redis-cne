...
var IndexToConfig = require('./index-to-config');
...
module.exports = CoreObject.extend({
  init: function() {
    ...
    this.memcached    = require('./memcached')(this.env);
    this.speedcurve   = require('./speedcurve')(this.env);
    this.hipchat      = require('./hipchat')(this.env);
    this.lockerGuy    = new LockerGuy({
      config: this.config,
  ...
  activate: function(revisionKey) {
    var isLocalActivation = /development|local/.test(this.env);
    var isProductionActivation = /production/.test(this.env);

    if (!revisionKey) {
      return this._printErrorMessage(this._noRevisionPassedMessage());
    };

    if (isProductionActivation && !process.env['HIPCHAT_API_KEY']) {
      return this._printErrorMessage(this._noHipchatKeyMessage());
    }
    ...
    return new RSVP.Promise(function(resolve, reject) {
      _self.lockerGuy._locked()
        .then(function(lockedMessage) {
          if (lockedMessage) {
            return reject(lockedMessage);
          }
          return true;
        })
        .then(function() {
    ...
    .then(function() {
      if (isProductionActivation) {
        return _self.hipchat.notify(revisionKey)
          .then(function(data) {
            _self.ui.writeLine('\nActivation logged to Hipchat!!\n\n');
            return true;
          })
      } else {
        return true;
    ...
    .catch(function(reason) {
      ...
      if (error === 'not found in redis') {
        ...
        return this._printErrorMessage(reason);
      }
    }.bind(this));
  },
  ...
  _noHipchatKeyMessage: function() {
    var err = '\nError! No hipchat api key detected.\n\n' +
      white('Please add ') +
      green('`export HIPCHAT_API_KEY=<itsasecret>`') +
      white(' to your environment!');
